...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        loss = np.sum(-y_true*np.log(probability))/num_train + 0.5*reg*np.sum(W1*W1) + 0.5*reg*np.sum(W2*W2)
        ...
        ds /= num_train
        db2 = ds.sum(axis=0)
        dW2 = np.dot(h_out.T, ds) + reg*W2
        dh_out = ds.dot(W2.T)
        dh_out[h_out<=0] = 0
        db1 = dh_out.sum(axis=0)
        dW1 = np.dot(X.T, dh_out) + reg*W1
        grads['W1'] = dW1
        grads['b1'] = db1
        grads['W2'] = dW2
        grads['b2'] = db2
